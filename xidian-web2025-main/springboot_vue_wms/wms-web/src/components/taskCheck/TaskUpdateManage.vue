...
<template>
  <div>
    <h1 style="padding-top: 20px;padding-bottom: 20px;text-align: center">下面是您的任务，工作辛苦了，祝您工作顺利！</h1>
    <!-- 搜索栏 -->
  ...
  data() {
    return {
      ...
      form: {
        ...
        description: "",
        status: 0,
        creatorId: this.$store?.state?.user?.id || 1, // 当前登录用户id，实际使用时应自动填充
        assigneeName: "",
        deadline: ""
      },
      rules: {
    ...
    loadTasks() {
      ...
      this.$axios.post(this.$httpUrl + "/task/listPageUpdate", {
        ...
        param: {
          ...
          assigneeName: user.name // 使用当前登录用户名
        }
