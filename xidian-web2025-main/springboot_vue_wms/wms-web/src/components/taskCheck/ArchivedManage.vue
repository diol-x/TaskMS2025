...
<template>
  <div>
    <h1 style="padding-top: 20px;padding-bottom: 20px;text-align: center">您好，下面的任务已完成审批归档，仅供您查阅！</h1>
    <!-- 搜索栏 -->
    ...
    >
      ...
      <el-table-column prop="assigneeName" label="分配人" width="160" />
      <el-table-column prop="deadline" label="截止时间" width="200" />
<!--      <el-table-column prop="operate" label="操作" width="140">-->
<!--        <template slot-scope="scope">-->
<!--          <el-button size="small" type="success" @click="openEditDialog(scope.row)">审批归档</el-button>-->
<!--          &lt;!&ndash;                <el-popconfirm&ndash;&gt;-->
<!--          &lt;!&ndash;                    title="确定删除该任务吗？"&ndash;&gt;-->
<!--          &lt;!&ndash;                    @confirm="del(scope.row.id)"&ndash;&gt;-->
<!--          &lt;!&ndash;                    style="margin-left: 5px;"&ndash;&gt;-->
<!--          &lt;!&ndash;                >&ndash;&gt;-->
<!--          &lt;!&ndash;                  <el-button slot="reference" size="small" type="danger">删除</el-button>&ndash;&gt;-->
<!--          &lt;!&ndash;                </el-popconfirm>&ndash;&gt;-->
<!--        </template>-->
<!--      </el-table-column>-->
    </el-table>
        ...
        >
          <el-form ref="form" :model="form" label-width="80px">
            ...
            <el-form-item label="归档备注">
              <el-input v-model="form.archiveRemark" type="textarea"></el-input>
            </el-form-item>
          </el-form>
...
export default {
  ...
  methods: {
    ...
    confirmArchive() {
      this.$confirm('确认归档该任务?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$axios.post(this.$httpUrl + "/task/update", {
          id: this.form.id,
          status: 6 // 将状态改为6表示已归档
          ...
          if (res.code === 200) {
            ...
            this.$message.error(res.msg || "归档失败");
          }
        });
      }).catch(() => {
...
//     // 登录时
//     this.$axios.post(this.$httpUrl + '/user/login', { no, password }).then(res => {
//       if (res.data.code === 200) {
//         // 存储用户信息
//         localStorage.setItem('user', JSON.stringify(res.data.data.user));
//         // 或 this.$store.commit('setUser', res.data.data.user)
//       }
//     });
//
// // 需要用户名时
...
//     const username = user.name; // 用户名
    ...
    loadTasks() {
      ...
      this.$axios.post(this.$httpUrl + "/task/listPageUpdate", {
        pageSize: this.pageSize,
        pageNum: this.pageNum,
        param: {
          title: this.searchTitle,
          status: 6, // 只查询已完成的任务(状态为5)
          creatorId: user.id // 只查询当前用户创建的任务
        }
      }).then(res => res.data).then(res => {
        if (res.code === 200) {
          ...
          this.$message.error(res.msg || "获取任务数据失败");
        }
      }).catch(error => {
